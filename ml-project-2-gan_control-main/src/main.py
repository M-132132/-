...
ap.add_argument("-e", "--num_episodes", type=int, default=100,
                help=("Number of episodes to run simulation for. Default 100"))  #强化学习中的训练轮数，训练完所有的轮数完成一次仿真步数
...
# create a directory inside experiment directory to store tensorboard logs  创建tensorboard目录
...
RL_Agent = DQNAgent(device, num_node_features, n_actions, ref_vx, **hyperparams)    #初始化深度DQN智能体用于强化学习任务
...
param_log_file = os.path.join(exp_log_dir, 'experiment_parameters.json')    #exp_log_dir代表实验的目录  os.path.join安全拼接路径
with open(param_log_file, 'w', encoding='utf-8') as f:
    json.dump(vars(args), f, ensure_ascii=False, indent=4)                  #将命令行参数对象 args 转换为字典  将字典写入JSON文件：ensure_ascii=False：允许保存非ASCII字符（如中文）。indent=4：缩进4空格，提升文件可读性。
    ...
    while runSimulation:
        ...
        if (i - i_crit) % f_controller == 0:
            ...
            if args.display_simulation:
                ...
                fig = plotScene(feature_map_i, scenarioADV, vehicleADV, vehList)
                plt.show()
               # fig_cankao = plotScene(refxL_out, refxR_out, refxT_out,vehicleADV)
                plt.show()
                plt.pause(0.02)
