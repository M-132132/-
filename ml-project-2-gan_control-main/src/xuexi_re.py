...
def main():
    ...
    N_sim = int(tsim / dt)
    RL_Agent = DQNAgent(device, num_node_features, n_actions, ref_vx, **hyperparams)       # 初始化RL Agent
    writer = SummaryWriter(log_dir=tensorboard_dir)             # 初始化TensorBoard
    ...
    vehicleADV, F_x_ADV, q_ADV_decision = initialize_vehicle(dt, N)     # 初始化车辆
    scenarioADV, scenarioTrailADV, roadMin, roadMax, laneCenters = initialize_scenarios(vehicleADV, N)     # 初始化场景
    vx_ego = 60 / 3.6           # 设置车辆初始位置
    ...
    for j in range(N_episodes):
        ...
        # 仿真初始化
        x_xunlian = DM(int(vehicleADV.nx), 1)
        ...
        # 保存模型和记录数据
        RL_Agent.save_model(os.path.join(exp_dir, 'model_checkpoint.pt'))
        # traffic.reset()
        i_overall = i
