...
class combinedTraffic:
    ...
    def getReference(self):
        ...
        for i in range(self.Nveh):
              ...
              # ref = self.vehicles[i].getReference()
              # if ref.ndim == 1:
              #     reference[:, i] = ref
              # else:
              #     reference[:, i] = ref.flatten()  # 或者使用reshape(-1)
        return reference
...
class vehicleSUMO:
    ...
    def getReference(self):
        ...
        """返回车辆的参考轨迹点"""
        # 确保nearP和farP是numpy数组
        # print(f"\n===== getReference() 调用 =====")
        # print(f"self.farP 类型: {type(self.farP)}")
        # print(f"self.farP 长度: {len(self.farP)}")
        # # 检查每个元素的类型和内容
        # for i, item in enumerate(self.farP):
        #     print(f"  元素 {i}: 类型={type(item)}, 内容={item}")
        #     if isinstance(item, np.ndarray):
        #         print(f"    数组形状: {item.shape}")
        # print(f"\n===== getReference() 调用 =====")
        # print(f"self.nearP 类型: {type(self.nearP)}")
        # print(f"self.nearP 长度: {len(self.nearP)}")
        # # 检查每个元素的类型和内容
        # for i, item in enumerate(self.nearP):
        #     print(f"  元素 {i}: 类型={type(item)}, 内容={item}")
        #     if isinstance(item, np.ndarray):
        #         print(f"    数组形状: {item.shape}")
        self.farP = [float(item) if isinstance(item, casadi.DM) else item for item in self.farP]
        self.nearP = [float(item) if isinstance(item, casadi.DM) else item for item in self.nearP]
        nearP = np.array(self.nearP)
        farP = np.array(self.farP)

        # 检查数组维度
        if nearP.ndim == 1 and farP.ndim == 1:
            # 都是一维数组，直接拼接并调整为列向量
            return np.hstack((nearP, farP)).reshape(-1, 1)
        elif nearP.ndim == 2 and farP.ndim == 2:
            # 都是二维数组，按行拼接
            return np.vstack((nearP, farP))
        else:
            # 混合维度，尝试调整后拼接
            if nearP.ndim == 1:
                nearP = nearP.reshape(1, -1)
            if farP.ndim == 1:
                farP = farP.reshape(1, -1)
            return np.vstack((nearP, farP))
